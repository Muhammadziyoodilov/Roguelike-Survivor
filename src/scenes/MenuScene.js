...
class MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Инициализация Swiper.js модального окна выбора арены
        if (window.MapSwiperModal) {
            MapSwiperModal.init(this);
        }
    ...
    closeCurrentModal() {
        if (window.MapSwiperModal && window.MapSwiperModal.isOpen()) {
            window.MapSwiperModal.close();
        }
    ...
    // --- МОДАЛЬНОЕ ОКНО ВЫБОРА КАРТЫ (3D SWIPER.JS) ---
    openMapSelectModal(selectedHeroId) {
        this.closeCurrentModal();
        if (window.MapSwiperModal) {
            window.MapSwiperModal.open(selectedHeroId);
        }
    }
}
